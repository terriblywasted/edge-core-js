...
import { cancelOtpReset, disableOtp, enableOtp } from '../login/otp'
import {
  changePassword,
  checkPassword,
  deletePassword
...
import { makeDataStoreApi, makePluginDataApi } from './data-store-api'
...
export function makeAccountApi(ai: ApiInput, accountId: string): EdgeAccount {
  ...
  const out: EdgeAccount = {
    ...
    get otpResetDate(): string | undefined {
      lockdown()
      const { loginTree } = selfState()
